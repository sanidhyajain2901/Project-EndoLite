...
class CustomDataset(torch.utils.data.Dataset):
    def __init__(self, data_dir):
        ...
        print(f"INFO: Using dummy dataset for path: {data_dir}")
        self.length = 128 # Dummy length
        self.num_classes = 4
...
class MobileNetV2Student(nn.Module):
    def __init__(self, num_classes=4):
        ...
        print("INFO: Using dummy MobileNetV2Student model.")
...
class DistillationTrainer:
    ...
    def distillation_loss(self, student_logits, teacher_logits, labels):
        ...
        soft_loss = -torch.sum(soft_targets * soft_prob) / soft_prob.size(0) * (self.temperature**2)
        
       
        hard_loss = self.criterion_ce(student_logits, labels)
        
      
        l1_norm = sum(torch.sum(torch.abs(param)) for param in self.student.parameters())
        lambda_l1 = 1e-5
    ...
    def train_epoch(self, epoch):
        ...
        for images, labels in pbar:
            ...
            pbar.set_postfix({
                'loss': f'{train_loss / (total / self.train_loader.batch_size):.3f}', 
